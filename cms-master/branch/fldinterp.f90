...
SUBROUTINE fld3_interp (ilon, ilat, idepth, nlon, nlat, ndepth, fld, &
                        weight,nweight, fillvalue,value, fail, landFlag)
 ...
 logical (kind=log_kind) :: landinterp, bicubic, bilinear, trilinear,tricubic,valueZero
...
!      print *, i, " ", j, " ", k, ": ", fld3d(i,j,k)
      IF (fld3d(i,j,k) .gt. fillvalue) THEN
       tricubic = .false.
...
!**************************************************************
!Interpolates sea surface height to an arbitray point.
!Only interpolates in space not in time!
SUBROUTINE ssh_interp (ilon, ilat, nlon, nlat, fld, fillvalue,value, fail)
