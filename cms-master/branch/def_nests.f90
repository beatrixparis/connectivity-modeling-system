...
MODULE def_nests
 ...
 TYPE grid
  ...
  integer (kind=int_kind)             :: idm, jdm, kdm, tdm,time_step,tstart_yy, tstart_mm, tstart_dd
  integer (kind=int_kind),allocatable :: mask(:,:)
  logical (kind=log_kind)             :: dataExist, tilted            
  real (kind = real_kind)             :: fill_value
  real (kind = real_kind),allocatable :: w(:,:), lon(:), lat(:), depth(:), uvel(:,:,:,:), vvel(:,:,:,:), &
                                         wvel(:,:,:,:), temp(:,:,:,:), saln(:,:,:,:), dens(:,:,:,:), ssh(:,:,:)
  character(char_len)                 :: fnameold, time_units
