...
SUBROUTINE fld3_interp (ilon, ilat, idepth, nlon, nlat, ndepth, fld, &
 ...
 real (kind = real_kind) :: fld3d (4,4,4),fld2d(4,4),fr(4,4), fs (4), &
 ...
 logical (kind=log_kind) :: is_in_minmax_range, is_inside_bounds
...
!       set grid node offsets
        ...
        DO j = 1,4
          DO i = 1,4
            ...
            y = ilat_int + j - 2
            z = idepth_int
            ! In this nested-if, the index 'y' reaches values out of the bounds of the array 'fld'
            ! Temporary solution: Check index and bypass if out of bounds
           ...
           !.and. y .le. ymax) THEN
              ...
              IF (nests(ngrid)%fill_value .eq. 0.) THEN
                ...
                ENDDO
              ELSE
                IF (nweight .eq. 2 .and. fld(new_x,y,z,1) .ge. fillvalue &
                .and. fld(new_x,y,z,2) .lt. fillvalue) THEN
           ...
           DO i = 1,4
	         ...
	         IF (is_in_minmax_range .and. is_inside_bounds) THEN
	           ...
	           IF (nests(ngrid)%fill_value .eq. 0.) THEN
	             ...
	             ENDDO
	           ELSE
                 IF (nweight .eq. 2 .and. abs(fld(new_x,y,z,1)) .ge. &
                 fillvalue .and. abs(fld(new_x,y,z,2)) .lt. fillvalue) THEN
...
SUBROUTINE fld2d_interp (ilon, ilat, nlon, nlat, fld, fillvalue, numbergrid, value, fail)
