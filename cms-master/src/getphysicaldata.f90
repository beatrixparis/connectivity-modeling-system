...
SUBROUTINE getphysicaldata(time_t, basedate)
 ...
 integer (kind=int_kind)         :: ncId,i,frac_tstep,numberFiles,dmonth, yyyy, dd, mm, &
...
!   read U-velocity
    CALL nc_open(trim(fname(UAx,fileNumber)),ncId)       
    CALL nc_read4d(fname(UAx,fileNumber), ncId, trim(nests(i)%uname),nests(i)%uvel(:,:,:,fileNumber), &
                   nests(i)%idm(UAx),nests(i)%jdm(UAx),nests(i)%kdm(UAx),1,snapshotvec)
    CALL nc_close(fname(UAx,fileNumber), ncId) 

!   read V-velocity
    CALL nc_open(trim(fname(VAx,fileNumber)),ncId)       
    CALL nc_read4d(fname(VAx,fileNumber), ncId, trim(nests(i)%vname),nests(i)%vvel(:,:,:,fileNumber), &
                   nests(i)%idm(VAx),nests(i)%jdm(VAx),nests(i)%kdm(VAx),1,snapshotvec)
...
!   read W-velocity
    IF (AxUsed(3).eqv..true.) THEN
     ...
     IF (nc_exists(ncId, trim(nests(i)%wname))) THEN
      CALL nc_read4d(fname(WAx,fileNumber), ncId, trim(nests(i)%wname),nests(i)%wvel(:,:,:,fileNumber), &
                     nests(i)%idm(WAx),nests(i)%jdm(WAx),nests(i)%kdm(WAx),1,snapshotvec)
      wExist = .true.
     ENDIF
...
!    read density
     IF (nc_exists(ncId, trim(nests(i)%densname))) THEN
      CALL nc_read4d(fname(QAx,fileNumber), ncId, trim(nests(i)%densname),nests(i)%dens(:,:,:,fileNumber), &
                     nests(i)%idm(QAx),nests(i)%jdm(QAx),nests(i)%kdm(QAx),1,snapshotvec)
      densExist = .true.
     ENDIF
!    read temperature
     IF (nc_exists(ncId, trim(nests(i)%tempname))) THEN
      CALL nc_read4d(fname(QAx,fileNumber), ncId, trim(nests(i)%tempname),nests(i)%temp(:,:,:,fileNumber), &
                     nests(i)%idm(QAx),nests(i)%jdm(QAx),nests(i)%kdm(QAx),1,snapshotvec)
      tempExist = .true.
     ENDIF
!    read salinity
     IF (nc_exists(ncId, trim(nests(i)%salnname))) THEN
      CALL nc_read4d(fname(QAx,fileNumber), ncId, trim(nests(i)%salnname),nests(i)%saln(:,:,:,fileNumber), &
                     nests(i)%idm(QAx),nests(i)%jdm(QAx),nests(i)%kdm(QAx),1,snapshotvec)
      salExist = .true.
     ENDIF
!    read sea surface height
     IF (nc_exists(ncId, "ssh")) THEN
      CALL nc_read3d(fname(QAx,fileNumber), ncId, "ssh",nests(i)%ssh(:,:,fileNumber), &
...
!   call theta to get density
    IF (densExist .eqv. .false.) THEN
     IF (tempExist .and. salExist) THEN
       CALL calc_dens(nests(i)%temp(:,:,:,fileNumber),nests(i)%saln(:,:,:,fileNumber),nests(i)%dens(:,:,:,fileNumber), &
